...
public sealed class OneModelCallPerThreadInvariantTests
{
    [Fact]
    public async Task NeverRunsTwoConcurrentModelCalls_ForSameThread()
    {
        ...
        // One prompt is enough: main will spawn child, child will run, and parent will be woken multiple times.
        ...
        factory.Chat.MaxConcurrentMain.Should().Be(1);
        factory.Chat.MaxConcurrentChild.Should().Be(1);
    ...
    {
        public ConcurrencyDetectingChatClient Chat { get; } = new();
        ...
        public Task<NewSessionResponse> NewSessionAsync(NewSessionRequest request, CancellationToken cancellationToken)
            => Task.FromResult(new NewSessionResponse { SessionId = "ses_conc_guard", Modes = null, ConfigOptions = new List<SessionConfigOption>() });

        public IAcpSessionAgent CreateSessionAgent(string sessionId, IAcpClientCaller client, IAcpSessionEvents events)
        {
            var rootDir = Path.Combine(Path.GetTempPath(), "harness-acp-conc-guard-tests", Guid.NewGuid().ToString("N"));
    ...
    private sealed class ConcurrencyDetectingChatClient : MeaiIChatClient
    {
        private int _mainInFlight;
        private int _childInFlight;
        public int MaxConcurrentMain { get; private set; }
        public int MaxConcurrentChild { get; private set; }

        private bool _mainToolsDone;
        private bool _childToolsDone;
        private int _mainContinuation;
        ...
        public IAsyncEnumerable<MeaiChatResponseUpdate> GetStreamingResponseAsync(IEnumerable<MeaiChatMessage> messages, MeaiChatOptions? options = null, CancellationToken cancellationToken = default)
        {
            ...
            {
                var inFlight = Interlocked.Increment(ref _mainInFlight);
                MaxConcurrentMain = Math.Max(MaxConcurrentMain, inFlight);

                try
                {
                    _mainToolsDone = true;
                    // Hold the call open briefly so concurrency would be observable if it existed.
                    ...
                    yield return new MeaiChatResponseUpdate
                    {
                        Contents = new List<MeaiAIContent>
                        {
                            ...
                            new MeaiFunctionCallContent("call_m_1", "thread_start", new Dictionary<string, object?> { ["name"] = "child", ["context"] = "fork", ["mode"] = "multi", ["message"] = "do work", ["delivery"] = "immediate" }),
                        }
                    };
                }
                finally
                {
                    Interlocked.Decrement(ref _mainInFlight);
                }
            }
            ...
            async IAsyncEnumerable<MeaiChatResponseUpdate> Main_Text_ContinueAfterIdle()
            {
                ...
                try
                {
                    _mainContinuation++;
                    await Task.Delay(25, cancellationToken);
                    yield return new MeaiChatResponseUpdate { Contents = new List<MeaiAIContent> { new MeaiTextContent($"continued:{_mainContinuation}") } };
                }
                finally
                {
                    Interlocked.Decrement(ref _mainInFlight);
            ...
            {
                var inFlight = Interlocked.Increment(ref _childInFlight);
                MaxConcurrentChild = Math.Max(MaxConcurrentChild, inFlight);
                ...
                {
                    Interlocked.Decrement(ref _childInFlight);
                }
            }
