...
public sealed class AgentShellExecuteClientDriveTests
{
    ...
    [Fact]
    public async Task AgentShellExecute_ClientDrive_WhenFsReadCapabilityMissing_ReturnsError()
    {
        var cwd = "/repo/cwd";
    ...
    [Fact]
    public async Task AgentShellExecute_ClientDrive_WhenFsWriteCapabilityMissing_ReturnsError()
    {
        var cwd = "/repo/cwd";
        var storeRoot = Path.Combine(Path.GetTempPath(), "harness-clientdrive-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(storeRoot);
        ...
        var res = await RunPs(exec, state, toolId: "t1",
            script: "Set-Content -Path client:\\demo.txt -Value 'x'");
