...
public sealed class AcpThreadNewCreatesThreadBeforeInboxIsProcessedIntegrationTests
{
    [Fact]
    public async Task ThreadNew_CreatesThreadBeforeChildInboxMessageIsProcessed()
    {
        var sessionId = "ses_thread_new_order";
        var root = Path.Combine(Path.GetTempPath(), "harness-thread-new-order", Guid.NewGuid().ToString("N"));
        ...
        // Assert: child thread exists and has the inter-thread message enqueued.
        var threadStore = new JsonlThreadStore(root);
        var child = threadStore.ListThreads(sessionId)
            .Single(t => t.ParentThreadId == ThreadIds.Main && t.ThreadId != ThreadIds.Main);

        var committed = threadStore.LoadCommittedEvents(sessionId, child.ThreadId);
        ...
        public IAsyncEnumerable<MeaiChatResponseUpdate> GetStreamingResponseAsync(IEnumerable<MeaiChatMessage> messages, MeaiChatOptions? options = null, CancellationToken cancellationToken = default)
        {
            ...
            async IAsyncEnumerable<MeaiChatResponseUpdate> Tools()
            {
                yield return new MeaiChatResponseUpdate
                {
                    ...
                    {
                        new MeaiFunctionCallContent("call_0", "thread_new", new Dictionary<string, object?>
                        {
                            ["message"] = "child hello",
                            ["delivery"] = "immediate",
                        }),
