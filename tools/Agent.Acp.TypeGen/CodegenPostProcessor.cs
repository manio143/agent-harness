...
public static class CodegenPostProcessor
{
    ...
    public static string PostProcessGeneratedCode(JsonSchema schema, string code)
    {
        var patchSpecs = new (string TargetDefName, string PlaceholderTypePattern, string ReplacementTypeName)[]
        {
            ...
            // Stop reason is a string-union in schema; we model it as a context-specific wrapper type.
            ("StopReason", "StopReason\\d*", "PromptResponseStopReason"),
            ("StopReason", "StopReason", "PromptResponseStopReason"),
        };
        ...
        // We intentionally remove the StopReason type declaration emitted by NJsonSchema.
        // It is a JSON Schema string-union, and we provide a context-specific wrapper type
        // (PromptResponseStopReason) generated by UnionGen.
        code = RemoveStopReasonDeclaration(code);

        // StopReason removal can leave a stray extra closing brace; clean it up deterministically.
        code = code.Replace("\n    }\n\n    }\n\n    /// <summary>\n    /// Embed a terminal", "\n    }\n\n    /// <summary>\n    /// Embed a terminal");
    ...
    private static string RemoveStopReasonDeclaration(string code)
    {
        // StopReason is a schema string-union and NJsonSchema emits it as a partial class.
        // Remove the entire declaration including the doc comment to avoid leaving stray braces.
        var re = new Regex(
            "\\n\\s*///\\s*<summary>\\s*\\r?\\n\\s*///\\s*Reasons why an agent stops processing a prompt turn\\.[\\s\\S]*?\\n\\s*\\}[\\t ]*\\r?\\n",
            RegexOptions.Compiled);

        return re.Replace(code, "\n");
    }
