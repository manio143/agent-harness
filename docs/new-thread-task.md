...
- [x] Phase 5 — Integration/sample updates
  ...
  - Centralized inbox meta key names in `ThreadInboxMetaKeys` (no more stringly-typed meta access)
  ...
  - Hardened inbox JSONL deserialization: tolerate missing optional fields on `thread_inbox_message_enqueued` and `thread_inbox_message_dequeued` (and back-compat variants) with sensible defaults
